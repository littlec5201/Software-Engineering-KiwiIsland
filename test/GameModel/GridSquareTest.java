...
public class GridSquareTest extends junit.framework.TestCase
{
    ...
    @Test
    public void testGetOccupantNameNoOccupants(){
        assertEquals("",emptySquare.getOccupantName() );
    }
    @Test
    public void testGetOccupantNameSingleOccupant(){
        assertEquals("apple",occupiedSquare.getOccupantName() );
    }
    @Test
    public void testGetOccupantNameMultipleOccupants(){
        // Add another occupant
        Tool trap = new Tool(position, "Trap", "A predator trap", 1.0, 2.0);
        occupiedSquare.addOccupant(trap); 
        // Add a third occupant
        Predator possum = new Predator(position, "Possum", "A log tailed possum"); 
        occupiedSquare.addOccupant(possum);          
        String stringRep = occupiedSquare.getOccupantName();
        assertEquals(15, stringRep.length());    
        assertTrue(stringRep.contains("Trap"));
        assertTrue(stringRep.contains("apple"));
        assertTrue(stringRep.contains("Possum"));
    }
    @Test
