...
public class PlayerTest extends junit.framework.TestCase
{
   ...
   Food sandwich;
    ...
    protected void setUp()
    {
        ...
        player = new Player(playerPosition,"Anthony Le",25.0, 15.0, 20.0);
        sandwich = new Food(playerPosition, "sandwich", "A tasty sandwich", 1.0, 2.0, 1.5);        
    }
    ...
    {
        assertEquals("Anthony Le", player.getName());
    }
    ...
    //sukim
    @Test 
    public void testHasSuperTrap(){
        Tool superTrap = new Tool(playerPosition, "SuperTrap", "A SuperTrap for predators", 2.0,3.0);
        player.collect(superTrap);
        assertTrue(player.hasSuperTrap());
    }
    
    @Test
    public void testHasNoSuperTrap(){
         assertFalse(player.hasSuperTrap());
    }
    
    @Test
    public void testGetSuperTrap(){
        Tool superTrap = new Tool(playerPosition, "SuperTrap", "A SuperTrap for predators", 2.0, 3.0);
        player.collect(superTrap);
        assertEquals(player.getSuperTrap(), superTrap);
    }
